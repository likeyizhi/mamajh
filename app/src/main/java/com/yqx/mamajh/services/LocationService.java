package com.yqx.mamajh.services;
...
import com.yqx.mamajh.utils.LocationUtils;
...
//            country = LocationUtils.getCountryName(AppApplication.getInstance(), Double.parseDouble(latitude), Double.parseDouble(longitude));
//            locality = LocationUtils.getLocality(AppApplication.getInstance(), Double.parseDouble(latitude), Double.parseDouble(longitude));
            street = LocationUtils.getStreet(AppApplication.getInstance(), Double.parseDouble(latitude), Double.parseDouble(longitude));
            if (mOnGetLocationListener != null) {
                mOnGetLocationListener.getLocation(lastLatitude, lastLongitude, latitude, longitude, country, locality, street);
            }
        }
    ...
    public void onCreate() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                Looper.prepare();
                isSuccess = LocationUtils.register(AppApplication.getInstance(), 0, 0, mOnLocationChangeListener);
                Looper.loop();
            }
        }).start();
    }
