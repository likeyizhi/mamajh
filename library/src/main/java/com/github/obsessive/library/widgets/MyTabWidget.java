    ...
    public MyTabWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (null == mLabels || mLabels.length <= 0) {
            try {
                ...
                TLog.i(TAG, MyTabWidget.class.getSimpleName() + " 出错");
            }
            a.recycle();
            return;
        }
    ...
    private void init(final Context context) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                ...
                itemName.setTextColor(Color.rgb(212, 2, 26));
//				view.setBackgroundColor(Color.rgb(240, 241, 242));
            } else {
                itemName.setChecked(false);
                itemName.setTextColor(Color.rgb(155, 155, 155));
    ...
    public void setTabsDisplay(Context context, int index) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((Integer) (checkedTextView.getTag()) == index) {
                TLog.i(TAG, mLabels[index] + " is selected...");
...
//				mViewList.get(i).setBackgroundColor(Color.rgb(250, 250, 250));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        height = Math.max(this.getBackground().getIntrinsicHeight(),
                heightSpecSize) + DensityUtils.dip2px(getContext(), 5);
...
//				mViewList.get(i).setBackgroundColor(Color.rgb(250, 250, 250));
    ...
    public void setDirectClickItem(int index) {
        if (index > mLabels.length) {
            try {
                throw new CustomException("索引超出底部菜单的数量...");
            } catch (CustomException e) {
        ...
        mViewList.get(index).performClick();
    }
